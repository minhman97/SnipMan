...
public class SnippetTagConfiguration : IEntityTypeConfiguration<SnippetTag>
{
    public void Configure(EntityTypeBuilder<SnippetTag> builder)
    {
        ...
        builder.HasOne<Tag>(st => st.Tag).WithMany(t => t.Snippets).HasForeignKey(st => st.TagId);
        builder.HasOne<Snippet>(st => st.Snippet).WithMany(t => t.Tags).HasForeignKey(st => st.SnippetId);
    }
}